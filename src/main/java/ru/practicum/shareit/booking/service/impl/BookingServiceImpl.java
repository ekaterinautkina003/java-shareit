...
public class BookingServiceImpl implements BookingService {
  ...
  public List<BookingDto> getBookingsByUser(Long userId, String state) {
    ...
    try {
      ...
      result = findBookingsByUserIdAndStatus(userId, status);
  ...
  @Override
  public List<BookingDto> getBookingStatusByOwner(Long userId, String state) {
    userRepository.findById(userId)
            .orElseThrow(NotFoundException::new);
    List<Booking> result;
    try {
      RequestBookingStatus status = RequestBookingStatus.valueOf(state);
      result = findBookingsByOwnerIdAndStatus(userId, status);
    } catch (IllegalArgumentException exception) {
      throw new UnknownStateException(state);
    }
    return result.stream()
